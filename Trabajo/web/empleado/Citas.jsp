                ...
                <tbody>
                    ...
                    <tr>
                      ...
                      <%
                        ...
                        EmpleBD empleado = new EmpleBD();
                        empleado.setConexion(conexion);
                        Vector colegas = empleado.misColegas(empresa);
                        if(colegas != null  &&  !colegas.isEmpty()){
                            for (int i=0; i < colegas.size();i++){
                                Empleado comp=(Empleado)colegas.elementAt(i); %>
                                <input type="checkbox" name="emp" value=<%=comp.getNick()%> /> <%=comp.getNick()%>
                                <br>
                      <%
                            }
                        }
...
%>
                    ...
                    <tr >
                      ...
                      <%
                        EmpleBD empleado2 = new EmpleBD();
                        empleado2.setConexion(conexion);
                        
                        Vector lideres = empleado2.getLideres(empresa);
                        if(lideres != null  &&  !lideres.isEmpty()){
                            for (int j=0; j < lideres.size();j++){
                            Empleado lider=(Empleado)lideres.elementAt(j);
%>
                            <input type="checkbox" name="emp" value=<%=lider.getNick()%> /> <%=lider.getNick()%>
                            <br>
                      <%}
                }
                else{ 
